...
! div_h . u_h = d(u_theta)/d theta + cot(theta) * u_theta + 1/sin(theta) * d(u_phi)/d phi
   ...
   REAL( KIND = LONG_REAL) ::  U_H(1:,0:,1:),  DIV_H_U_H(1:,0:), ALF(1:,1:), DALF(1:,1:), ONE_DIV_SIN_THETA(1:), COSTHETA_GRID(1:)
   TYPE (HARMONIC_STRUCTURE) :: HARMONICS(1:)
 
    INTEGER :: INDEX, I_THETA, I_PHI, J, LOWER_HARMONIC, UPPER_HARMONIC
    REAL( KIND = LONG_REAL) :: PHI, DERV_PHI_DEP, PHI_DEP
...
! Evaluates u_H on the defined (theta,phi) grid
   ...
   REAL( KIND = LONG_REAL) ::  U_H(1:,0:,1:), ALF(1:,1:), DALF(1:,1:), ONE_DIV_SIN_THETA(1:), SPEC_TOR(1:),SPEC_POL(1:), DIV_H_U_H(1:,0:)
   TYPE (HARMONIC_STRUCTURE) :: HARMONICS(1:)
...
! poloidal
   ...
   DIV_H_U_H(I_THETA, I_PHI) = DIV_H_U_H(I_THETA, I_PHI) - SPEC_POL(I_HARMONIC) * ALF(I_THETA, INDEX) * PHI_DEP * REAL( HARMONICS(I_HARMONIC)%L * ( HARMONICS(I_HARMONIC)%L + 1), KIND = LONG_REAL ) / CMB_RADIUS
...
! Writes the flow to a file that GMT can read. Expects u to describe a flow in units of km/yr
      ...
      REAL( KIND = LONG_REAL) :: EVEC_TOR(1:), EVEC_POL(1:), SCALE_FACTOR
      TYPE ( HARMONIC_STRUCTURE ) :: HARMONICS(1:)

      REAL( KIND = LONG_REAL), ALLOCATABLE :: COSTHETA_GRID(:), ALF(:,:), DALF(:,:), ONE_DIV_SIN_THETA(:), U_H(:,:,:)
      REAL( KIND = EXTRA_LONG_REAL), ALLOCATABLE :: TEMP_THETA_TRANSFORM_ALF(:,:), TEMP_THETA_TRANSFORM_DALF(:,:)
      REAL( KIND = LONG_REAL) :: PHI, ANGLE_VEC, MOD_VEC, PHI_DEP, DERV_PHI_DEP
      INTEGER :: I_THETA, I_PHI, I_HARMONIC, INDEX
  
! setup grids
      ALLOCATE( COSTHETA_GRID(1: GMT_THETA))
      DO I_THETA = 1, GMT_THETA
       COSTHETA_GRID(I_THETA) = COS(  I_THETA / REAL( GMT_THETA + 1, KIND = LONG_REAL ) * Pi   )
      ENDDO
...
IMPLICIT NONE
INTEGER ::  GMT_THETA, GMT_PHI,  LMAX
REAL( KIND = LONG_REAL) :: EVEC_TOR(1:), EVEC_POL(1:)
TYPE ( HARMONIC_STRUCTURE ) :: HARMONICS(1:)
...
PHI = -Pi + Pi /REAL(GMT_PHI, KIND = LONG_REAL) + 2.0 * Pi / REAL(GMT_PHI, KIND = LONG_REAL) * (I_PHI-1)
...
REAL( KIND = LONG_REAL), ALLOCATABLE :: COSTHETA_GRID(:), ALF(:,:), DALF(:,:), U_H(:)
REAL( KIND = EXTRA_LONG_REAL), ALLOCATABLE :: TEMP_THETA_TRANSFORM_ALF(:,:), TEMP_THETA_TRANSFORM_DALF(:,:)
REAL( KIND = LONG_REAL) :: PHI, PHI_DEP, DERV_PHI_DEP,  ONE_DIV_SIN_THETA, RANDOM_NUMBERS( 1:2 ), THETA
...
REAL( KIND = LONG_REAL) :: EVEC_TOR(1:), EVEC_POL(1:), USQ(:,:), PHI_GRID(PHI_GRID_SIZE), COSTHETA_GRID(1:THETA_GRID_SIZE)
TYPE ( HARMONIC_STRUCTURE ) :: HARMONICS(1:)

REAL( KIND = LONG_REAL), ALLOCATABLE :: ALF(:,:), DALF(:,:), ONE_DIV_SIN_THETA(:), U_H(:,:,:)
REAL( KIND = EXTRA_LONG_REAL), ALLOCATABLE :: TEMP_THETA_TRANSFORM_ALF(:,:), TEMP_THETA_TRANSFORM_DALF(:,:)
REAL( KIND = LONG_REAL) :: PHI, ANGLE_VEC, MOD_VEC, PHI_DEP, DERV_PHI_DEP
INTEGER :: I_THETA, I_PHI, I_HARMONIC, INDEX
...
PHI = (I_PHI-1) * 2.0_LONG_REAL * Pi / REAL(PHI_GRID_SIZE, KIND = LONG_REAL)
...
REAL( KIND = LONG_REAL) :: EVEC_TOR(1:)
TYPE ( HARMONIC_STRUCTURE ) :: HARMONICS(1:)

REAL( KIND = LONG_REAL), ALLOCATABLE :: COSTHETA_GRID(:), ALF(:,:), DALF(:,:), ONE_DIV_SIN_THETA(:), STREAMFN(:,:)
REAL( KIND = EXTRA_LONG_REAL), ALLOCATABLE :: TEMP_THETA_TRANSFORM_ALF(:,:), TEMP_THETA_TRANSFORM_DALF(:,:)
REAL( KIND = LONG_REAL) :: PHI, ANGLE_VEC, MOD_VEC, PHI_DEP, DERV_PHI_DEP
INTEGER :: I_THETA, I_PHI, I_HARMONIC, INDEX
...
COSTHETA_GRID(I_THETA) = COS(  I_THETA / REAL( N_THETA + 1, KIND = LONG_REAL ) * Pi   )
ENDDO
...
DO I_PHI = 1, N_PHI
! splits up into equal segments away from the end points.
PHI = -Pi + Pi /REAL(N_PHI, KIND = LONG_REAL) + 2.0 * Pi / REAL(N_PHI, KIND = LONG_REAL) * (I_PHI-1)
...
!Also write in a centred grid for Python
  ...
  REAL( KIND = LONG_REAL) :: GAUSS_DOT(:), GAUSS(:)
  TYPE ( HARMONIC_STRUCTURE ) :: HARMONICS(1:)
  INTEGER :: LMAX_SV, LMAX_B

      REAL( KIND = LONG_REAL), ALLOCATABLE :: COSTHETA_GRID(:), ALF(:,:), DALF(:,:), ONE_DIV_SIN_THETA(:)
      REAL( KIND = EXTRA_LONG_REAL), ALLOCATABLE :: TEMP_THETA_TRANSFORM_ALF(:,:), TEMP_THETA_TRANSFORM_DALF(:,:)
      REAL( KIND = LONG_REAL) :: PHI,  PHI_DEP, DERV_PHI_DEP, B_DOT_LOCATION(3), B_LOCATION(3)
...
! setup grids
    ...
    OPEN(16, FILE = 'F_DOT_ES.DAT', STATUS = 'REPLACE', FORM = 'FORMATTED')
    OPEN(17, FILE = 'F_ES.DAT', STATUS = 'REPLACE', FORM = 'FORMATTED')

    DO I_PHI = 0, NPHI_GRID_CONTOUR_GMT-1
    DO I_THETA = 1, NTHETA_GRID_CONTOUR_GMT
    
    PHI =  I_PHI * 2.0_LONG_REAL * Pi / REAL(NPHI_GRID_CONTOUR_GMT, KIND = LONG_REAL)
    B_DOT_LOCATION(1:3) = 0.0_LONG_REAL
    B_LOCATION(1:3) = 0.0_LONG_REAL
...
! evaluate B and B_dot at Earth surface
    ...
    B_DOT_LOCATION(1) = B_DOT_LOCATION(1) + GAUSS_DOT(I) * ALF(I_THETA, INDEX_PLM) * PHI_DEP  * REAL(HARMONICS(I)%L + 1, KIND = LONG_REAL)
    ...
    B_LOCATION(1) = B_LOCATION(1) + GAUSS(I) *  ALF(I_THETA, INDEX_PLM) * PHI_DEP * REAL(HARMONICS(I)%L + 1, KIND = LONG_REAL)   
    ...
    ENDDO
    WRITE(16,'(2F8.2,ES15.5)'), PHI*180.0/Pi ,90.0d0-ACOS(COSTHETA_GRID(I_THETA))*180.0/Pi, SUM( B_DOT_LOCATION(:)  * B_LOCATION(:) ) / SQRT( SUM(B_LOCATION**2 ) ) 
    WRITE(17,'(2F8.2,ES15.5)'), PHI*180.0/Pi ,90.0d0-ACOS(COSTHETA_GRID(I_THETA))*180.0/Pi,  SQRT( SUM(B_LOCATION**2 ) ) 
...
OPEN(16, FILE = 'F_DOT_ES_CENTRED.DAT', STATUS = 'REPLACE', FORM = 'FORMATTED')
OPEN(17, FILE = 'F_ES_CENTRED.DAT', STATUS = 'REPLACE', FORM = 'FORMATTED')

DO I_PHI = 1, NPHI_GRID_CONTOUR_GMT
DO I_THETA = 1, NTHETA_GRID_CONTOUR_GMT

PHI = -Pi + Pi /REAL(NPHI_GRID_CONTOUR_GMT, KIND = LONG_REAL) + 2.0 * Pi / REAL(NPHI_GRID_CONTOUR_GMT, KIND = LONG_REAL) * (I_PHI-1)
B_DOT_LOCATION(1:3) = 0.0_LONG_REAL
B_LOCATION(1:3) = 0.0_LONG_REAL
...
OPEN(16, FILE = 'F_DOT_CS_CENTRED.DAT', STATUS = 'REPLACE', FORM = 'FORMATTED')
OPEN(17, FILE = 'F_CS_CENTRED.DAT', STATUS = 'REPLACE', FORM = 'FORMATTED')
...
INDEX_PLM = HARMONICS(I)%M * LMAX_SV +  (HARMONICS(I)%M * (3-HARMONICS(I)%M) ) /2 + 1 + (HARMONICS(I)%L - HARMONICS(I)%M)
B_DOT_LOCATION(1) = B_DOT_LOCATION(1) + GAUSS_DOT(I) * ALF(I_THETA, INDEX_PLM) * PHI_DEP  * REAL(HARMONICS(I)%L + 1, KIND = LONG_REAL) * (ES_RADIUS / CMB_RADIUS)**(HARMONICS(I)%L+2)
...
IF( HARMONICS(I)%L .LE. LMAX_B ) THEN !SV goes to higher L than B[obs], so exclude if index goes higher than required.
B_LOCATION(1) = B_LOCATION(1) + GAUSS(I) *  ALF(I_THETA, INDEX_PLM) * PHI_DEP * REAL(HARMONICS(I)%L + 1, KIND = LONG_REAL)* (ES_RADIUS / CMB_RADIUS)**(HARMONICS(I)%L+2)
...
REAL( KIND = LONG_REAL), ALLOCATABLE :: COSTHETA_GRID(:), ALF(:,:), DALF(:,:), ONE_DIV_SIN_THETA(:)
REAL( KIND = EXTRA_LONG_REAL), ALLOCATABLE :: TEMP_THETA_TRANSFORM_ALF(:,:), TEMP_THETA_TRANSFORM_DALF(:,:)
REAL( KIND = LONG_REAL) :: PHI,  PHI_DEP, DERV_PHI_DEP, B_DOT_LOCATION(3), B_LOCATION(3), BR_CS
INTEGER :: I_THETA, I_PHI, I_HARMONIC, INDEX_PLM, I
...
BR_CS = BR_CS + GAUSS(I) *  ALF(I_THETA, INDEX_PLM) * PHI_DEP * REAL(HARMONICS(I)%L + 1, KIND = LONG_REAL) * (ES_RADIUS / CMB_RADIUS)**(HARMONICS(I)%L+2)
...
!  Units should be in nT/yr
  ...
  INTEGER :: LMAX_SV, LMAX_B
      ...
      REAL( KIND = LONG_REAL), ALLOCATABLE :: COSTHETA_GRID(:), ALF(:,:), DALF(:,:), ONE_DIV_SIN_THETA(:)
      REAL( KIND = EXTRA_LONG_REAL), ALLOCATABLE :: TEMP_THETA_TRANSFORM_ALF(:,:), TEMP_THETA_TRANSFORM_DALF(:,:)
      REAL( KIND = LONG_REAL) :: PHI,  PHI_DEP, DERV_PHI_DEP, B_DOT_CS, B_DOT_ES, B_CS, B_ES
      INTEGER :: I_THETA, I_PHI, I_HARMONIC, INDEX_PLM, I
...
!  Units should be in nT
  ...
  REAL( KIND = LONG_REAL) :: GAUSS(:)
  TYPE ( HARMONIC_STRUCTURE ) :: HARMONICS(1:)
  INTEGER :: LMAX_B, NTHETA_GRID, NPHI_GRID

      REAL( KIND = LONG_REAL), ALLOCATABLE :: COSTHETA_GRID(:), ALF(:,:), DALF(:,:), ONE_DIV_SIN_THETA(:)
      REAL( KIND = EXTRA_LONG_REAL), ALLOCATABLE :: TEMP_THETA_TRANSFORM_ALF(:,:), TEMP_THETA_TRANSFORM_DALF(:,:)
      REAL( KIND = LONG_REAL) :: PHI,  PHI_DEP, DERV_PHI_DEP, B(1:3), THETA_MIN, PHI_MIN, F, F_MIN
      INTEGER :: I_THETA, I_PHI, I_HARMONIC, INDEX_PLM, I
...
! setup grids
      ...
      DO I_THETA = 1, NTHETA_GRID
       COSTHETA_GRID(I_THETA) = COS(  I_THETA / REAL( NTHETA_GRID + 1, KIND = LONG_REAL ) * Pi   )
      ENDDO
...
! evaluate B and B_dot at Earth surface
    ...
    B(1) = B(1) + GAUSS(I) *  ALF(I_THETA, INDEX_PLM) * PHI_DEP * REAL(HARMONICS(I)%L + 1, KIND = LONG_REAL)   
    ...
    IF( F < F_MIN ) THEN
    F_MIN = F
    THETA_MIN =  I_THETA / REAL( NTHETA_GRID + 1, KIND = LONG_REAL ) * 180 
