...
in the aacgmv2 repository and modify it with our coefficents. aacgmv2 somehow reads things
in the header that I am not aware of
"""
...
folder = '/Users/stefanomaffei/Documents/geomag/models/COVOBS.x2/cov-obs-x2_ensemble_real/dated_models_int/'

MF_file = folder +'mf_int_coefs.dat'
#SV_file = folder + 'gauss_SV_2015-2115'
 
out_file = folder + 'COV_OBSx2_MF4aacgmv2_from1590.txt'
...
# pure dipole: for test
#MFcoeffs[:,2:]=1


f = open(out_file,'w')
# header
f.write('# COV_OBSx,2 model\n')
f.write('# units of nT \n')
# 1st row
#f.write('c/s d o  ')
#for i in range(MFcoeffs.shape[0]):
#    f.write('MF  ')
#f.write('SV\n')
...
# 2nd row, years
#f.write('g/h n m ')
#for i in range(0,years.shape[0],5):
#    f.write(str(years[i]) +' ' )
#f.write(str(years[-1]) +'\n' )
# copy it from the magmodel file
...
# fill with the available coefficients, leave the rest to zero
#coeffs[:MFcoeffs.shape[0],1:MFcoeffs.shape[1]] = MFcoeffs[:,1:]

# axial dipole, for test
#MFcoeffs[:,2:]=0.1
...
for j in range(1,Nd): # cycle over the columns degree and order of SH
    ...
    f#or i in range(0,MFcoeffs.shape[0]): # print every 5 years
